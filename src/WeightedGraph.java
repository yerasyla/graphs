...
public class WeightedGraph<Vertex> {
    ...
    public Map<Vertex, List<Edge<Vertex>>> map = new HashMap<>();
    ...
    public void addEdge(Vertex source, Vertex dest, double weight) {
        ...
        ((LinkedList)map.get(source)).add(new Edge<>(source, dest, weight));

        if (undirected)
            ((LinkedList)map.get(dest)).add(new Edge<>(dest, source, weight));
